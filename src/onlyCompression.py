...
class DualAutoEncoder(nn.Module):
    ...
    def forward(self, x_train, training=True):
        ...
        if(training):
            ...
            return train_encoded, train_decoded, class_logits
        else:
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (dense_batch_train, batch_labels) in enumerate(get_batch(train_features, labels=train_labels, device=device)):
            ...
            train_encoded, train_decoded, class_logits = model(dense_batch_train, training=True)
            ...
            total_loss = reconstruction_loss_train + classification_loss_train
            total_loss.backward()
            optimizer.step()
